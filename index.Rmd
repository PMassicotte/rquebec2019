...
Maintenant que la fonction `scale_vector()` fonctionne, utilisons-là sur chaqune des colonnes du *data frame*.
...
Une deuxième amélioration: pourquoi ne pas **utiliser une boucle** qui permetterait de répéter l'opération plusieurs fois?
...
- La principale characteristique de la programmation fonctionnelle est **que des fonctions sont passées en paramètre** à d'autres fonctions.
...
- Le code est plus compate (plus facile à lire et comprendre).
...
L'exemple précédent peut se ré-écire en utilisant une fonction anonyme. Avec cette approche, nous n'avons pas besoin de fournir un nom de fonction.
...
# Selectionner seulement les colonnes numériques
...
Par example, on sait que la fonction `mean()` retourn une valeur numérique. Dans ce cas, on peut utiliser la fonction `map_dbl()` pour calculer la valeur moyenne de chacune des colonnes de `mtcars`.
...
.full-width[.content-box-gray[Séparez le jeux de données `mtcars` en fonction du nombre de cylindre. Pour chacun des groupes, faire un modèle linéaire `mpg` en fonction de `wt` (`lm(mpg ~ wt)`). Écrivez une fonction `fit()` pour faire le modèle linéraire.]]
...
## Mappage sur plusieurs arguments

Jusqu'ici, nous avons *mappé* avec une seule entrée. Mais souvent, vous avez plusieurs entrées que vous devez parcourir en parallèle. Pour y arriver, nous allons voir les fonctions `map2()` et `pmap()`.

Imaginons que nous voulions générer trois vecteurs de valeurs aléatoires.
...
map2_dbl(c(1, 2, 3), c(4, 5, 6), sum)

map2_dbl(c(1, 2, 3), c(4, 5, 6), ~.x + .y)

pmap_dbl(list(c(1, 2, 3), c(4, 5, 6), c(4, 5, 6)), ~..1 + ..2 + ..3)
...
## Exercice: real case scenario

---

## Lire le contenu d'un répertoire

Une tâche courrante consiste à lire plusieurs fichiers (CSV) dans un répertoire et de les combiner dans un seul *data frame*.
...
Figure pour explorer les données un peu.
...
- Les fonctions de `furrr` débutent toutes par `future_*()`.
...
Dans l'exercice précédent, nous devions calculer la moyenne de la colonne `duration` pour chaque mois de 2018. Disons maintenant que nous voulons calculer la moyenne de toutes les colonnes numériques.
...
- La fonction `group_by()` peut s'utiliser conjointement avec la fonction `summarise()` pour appliquer un calcule à un ensemble de groupes d'un *data frame*.
...
Calculer **les moyennes mensuelles** de toutes les colonnes numériques, en utilisant toutes les données de *bikeshare*. On peut diviser se problème en trois étapes:
...
files %>% 
  map_df(read_csv) %>% 
  group_by(month = lubridate::month(start_date)) %>% 
...
df %>% 
  ggplot(aes(x = hour, y = mean_duration, color = factor(month))) +
  geom_line() +
...
<!-- famille apply -> applique une fonction à... -->
