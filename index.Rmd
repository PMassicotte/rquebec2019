...
- Télédétection, marquage / suivi d'animaux
- Capteurs in situ automatisés (salinité, chla, ...)
...
En tant que scientifiques, vous devez apprendre et mettre en pratique les outils essentiels pour capturer, manipuler et partager des données.
...
Lire trois fichiers ayant la même structure.
...
Fonctionne très bien, mais il y a 4 répétitions. Où est le problème?
...
- La principale characteristique de la programmation fonctionne est **que des fonctions sont passées en paramètre** à d'autres fonctions.
...
L'exemple précédent peut se ré-écire en utilisant une fonction anonyme.
...
```{r}
map(df, mean, na.rm = TRUE) # Spécifier que la fonction "mean" doit utiliser na.rm = TRUE
```
...
Par example, on sait que la fonction `mean()` retourn une valeur numérique. Dans ce cas, on peut utiliser la fonction `map_dbl()` pour extraîre la classe de chacune des colonnes de `mtcars`.

```{r}
# Appliquer la fonction class à chacune des colonnes de mtcars
map_dbl(mtcars, mean) 
```
...
```{r}
res <- map_dfc(mtcars, as.character)
head(res)
